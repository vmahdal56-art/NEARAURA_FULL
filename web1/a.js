...
const SHARED_LOGIC = \`
  ...
  const ANTHEMS = {
    A1: { id: 'Bp1yD_bLXdM', title: 'ANTHEM_01', 
    ...
    A2: { id: 'nT1mElKoeHU', title: 'ANTHEM_02', 
          lyric_cz: 'BÝT MEDVÍDKEM... DÁVAT RADOST DĚTEM...', 
          lyric_en: 'TO BE A TEDDY BEAR... GIVING JOY TO THE KIDS...' },
    A3: { id: 'TcJ-wNmazHQ', title: 'ANTHEM_03', 
          lyric_cz: 'FINÁLNÍ ODPOČET. SADOVÁ 862. BOOTING...', 
          lyric_en: 'FINAL COUNTDOWN. SADOVA 862. BOOTING...' }
  };

  const LangSwitch = () => (
    <div className="fixed top-4 right-4 z-[100] flex gap-2 bg-black/90 border border-zinc-800 p-1.5 rounded-full shadow-2xl">
...
const GatewayPage = () => {
  ...
  return (
    <div className="h-screen w-full bg-[#050505] text-white flex flex-col font-sans overflow-hidden uppercase font-black relative">
      <style>{\\\` @keyframes marquee { 0% { transform: translateX(100%); } 100% { transform: translateX(-100%); } } .animate-marquee { display: inline-block; white-space: nowrap; animation: marquee 25s linear infinite; padding-left: 100%; } \\\`}</style>
      <LangSwitch />
      <div className="absolute inset-0 z-0 opacity-[0.05] pointer-events-none bg-[url('https://media.giphy.com/media/oEI9uWUPr9WUM/giphy.gif')]" />
      ...
      <div className="w-full text-center pt-10 pb-4 bg-black border-b border-zinc-900">
        <h1 className="text-3xl md:text-5xl italic tracking-tighter">
          <span className="text-[#ef4444]">{t("KONEC LŽÍ", "END OF LIES")}</span> <span className="text-white">⟶</span> <span className="text-[#10b981]">{t("ZROZENÍ PRAVDY", "REBIRTH OF TRUTH")}</span>
        </h1>
      </div>
    ...
    <div className="min-h-screen w-full bg-[#050505] text-white flex flex-col items-center p-6 font-mono relative pb-48 overflow-y-auto uppercase font-black">
      ...
      <div className="max-w-5xl w-full space-y-24">
        ...
        <div className="bg-white text-black p-12 border-8 border-black text-center relative shadow-[0_0_80px_rgba(255,255,255,0.4)]">
            <h2 className="text-6xl md:text-9xl font-[1000] tracking-tighter mb-4 uppercase">LITTLE DJANGO</h2>
